...
func parseDate(date string) (time.Time, error) {
	// Klikbca does not send back "YEAR" information. the hack is that we appeneded
	// the current year in front of the date string.
	//
	// The format  of the date is YYYY/DD/MM
	date = fmt.Sprintf("%s/%s", time.Now().Format("2006"), date)
	return time.Parse("2006/02/01", date)
}

func processSettlement(date time.Time, bookKeepType, outerHTML string) []settlementDetail {
	...
	for _, data := range strings.Split(outerHTML, "\n") {
		...
		formattedDate := date.Format("2006/02/01")
		id := generateID(formattedDate, bookKeepType, data)
...
func (klikBca klikBca) GetTodaySettlement() ([]settlementDetail, error) {
	...
	klikBca.colly.OnHTML(dailySettlementSelector, func(e *colly.HTMLElement) {
		...
		parsedDate, parseDateErr := parseDate(date)
		if err != nil {
			err = parseDateErr
			return
		}
		...
		settlement = append(settlement, processSettlement(parsedDate, bookKeepType, outerHTML)...)
	})
