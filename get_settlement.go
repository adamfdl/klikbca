...
type settlementDetail struct {
	// ID is generated by a formula of hash(bookKeepTypetype+htmlOfDescription)
	ID string `json:"id"`
	...
	// Amount has a format of e.g 20,000.00
	Amount string `json:"amount"`
}

func generateID(bookKeepType, outerHTML string) string {
	return fmt.Sprintf("%x", md5.Sum([]byte(bookKeepType+outerHTML)))
}

func processSettlement(bookKeepType, outerHTML string) []settlementDetail {
	...
	for _, data := range strings.Split(outerHTML, "\n") {
		...
		settlement = append(settlement, settlementDetail{
			ID:          generateID(bookKeepType, data),
...
func (klikBca klikBca) GetTodaySettlement() ([]settlementDetail, error) {
	...
	klikBca.colly.OnHTML(dailySettlementSelector, func(e *colly.HTMLElement) {
		...
		settlement = append(settlement, processSettlement(bookKeepType, outerHTML)...)
	})
